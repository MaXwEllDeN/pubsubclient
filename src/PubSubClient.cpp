...
uint32_t PubSubClient::readPacket(uint8_t* lengthLength) {
	...
	*/
    ...
    uint32_t length = 0; // remaining length
    uint8_t digit = 0; // single byte buffer
    uint16_t skip = 0; // if MQTTPUBLISH, = topic length + message id length (if qos>0)
    uint32_t start = 0; // =2 if MQTTPUBLISH, to skip the two already read topic length bytes in the for loop
        ...
        if (_client->available()) {
            ...
            if (len > 0) {
                ...
                if (type == MQTTPUBLISH) {
                    ...
                    this->buffer[0] = MQTTPUBREL | 2; // 7th bit of first byte must be set for PUBREL
                    _client->write(this->buffer, 4); // 2nd, 3rd and 4th bytes of PUBREL are the same of PUBREC
...
boolean PubSubClient::publish(const char* topic, const char* payload, uint8_t qos, boolean retained) {
    return publish(topic,(const uint8_t*)payload, payload ? strnlen(payload, this->bufferSize) : 0,qos, retained);
}
...
boolean PubSubClient::publish(const char* topic, const uint8_t* payload, unsigned int plength, uint8_t qos, boolean retained) {
    if (connected()) {
        if (this->bufferSize < MQTT_MAX_HEADER_SIZE + 2+strnlen(topic, this->bufferSize) + plength + (qos ? 2 : 0)) {
            // Too long
            return false;
        }
        // Leave room in the buffer for header and variable length field
...
boolean PubSubClient::publish_P(const char* topic, const char* payload, uint8_t qos, boolean retained) {
    return publish_P(topic, (const uint8_t*)payload, payload ? strnlen(payload, this->bufferSize) : 0, qos, retained);
}
